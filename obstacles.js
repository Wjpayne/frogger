    ...
    draw() {
        if (this.type === 'turtle') {
           ...
           ctx1.drawImage(turtle, this.frameX * 70, this.frameY * 70, 70, 70, this.x,  this.y, this.width, this.height);
               ...
               ctx5.drawImage(car, this.frameX * this.width, this.carType * this.height, grid *2, grid, this.x, this.y, this.width, this.height);
           }
...
function initObstacles () {
   ...
   for (let i =0; i < 2; i++) {
    ...
    for ( let i = 0; i < 2; i++) {
        let x = i * 400;
        carsArray.push(new Obstacle(x, canvas.height - grid * 4 - 20, grid * 2, grid, 1.5, 'car'));
    }
    ...
    for ( let i = 0; i < 2; i++) {
        let x = i * 350;
        logsArray.push(new Obstacle(x, canvas.height - grid * 5 -20, grid * 2, grid, -1.8, 'log' ));
...
// make sure obstacles reiterate across the screen when finished
...
function handleObstacles() {
    ...
    for (let i = 0; i < carsArray.length; i++) {
        if (collision(frogger, carsArray[i])) {
            ctx5.drawImage(collisions, 0, 100, 100, 100, frogger.x, frogger.y, 150, 150)
            resetGame();
        }
 
    }

    // fall off log or turtle, fall in water, or stay on log or turtle too long
